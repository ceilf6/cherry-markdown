...
/**
 * 强制展开所有可展开的内容，包括代码块和手风琴语法
 * @param {HTMLElement} cherryPreviewer 预览区域的DOM
 * @returns {function} 恢复函数，用于恢复到导出前的状态
 */
function expandAllContent(cherryPreviewer) {
  // 1. 强制展开所有代码块
  const codeBlocks = cherryPreviewer.querySelectorAll('.cherry-code-unExpand');
  const codeBlockStates = [];
  codeBlocks.forEach((codeBlock, index) => {
    codeBlockStates[index] = {
      element: codeBlock,
      wasUnExpanded: true,
    };
    codeBlock.classList.remove('cherry-code-unExpand');
    codeBlock.classList.add('cherry-code-expand');
  });

  // 2. 强制展开所有手风琴语法（details元素）
  const detailElements = cherryPreviewer.querySelectorAll('details');
  const detailStates = [];
  ...
  // 3. 处理图片懒加载 - 将data-src替换为src
  const lazyImages = cherryPreviewer.querySelectorAll('img[data-src]');
  const imageStates = [];
  lazyImages.forEach((img, index) => {
    ...
    if (dataSrc) {
      imageStates[index] = {
        element: img,
        hadDataSrc: true,
        originalSrc: /** @type {HTMLImageElement} */ (img).src,
      };
      /** @type {HTMLImageElement} */ (img).src = dataSrc;
      img.removeAttribute('data-src');
    }
  });

  // 4. 处理视频封面懒加载
  const lazyVideos = cherryPreviewer.querySelectorAll('video[data-poster]');
  const videoStates = [];
  lazyVideos.forEach((video, index) => {
    ...
    if (dataPoster) {
      videoStates[index] = {
        element: video,
        hadDataPoster: true,
        originalPoster: /** @type {HTMLVideoElement} */ (video).poster,
      };
      /** @type {HTMLVideoElement} */ (video).poster = dataPoster;
  ...
  return function restore() {
    // 恢复代码块状态
    codeBlockStates.forEach((state) => {
      if (state && state.wasUnExpanded) {
        state.element.classList.remove('cherry-code-expand');
        state.element.classList.add('cherry-code-unExpand');
      }
    });
    ...
    detailStates.forEach((state) => {
      if (state) {
        state.element.open = state.wasOpen;
      }
    });

    // 恢复图片懒加载状态
    imageStates.forEach((state) => {
      if (state && state.hadDataSrc) {
        state.element.setAttribute('data-src', /** @type {HTMLImageElement} */ (state.element).src);
        if (state.originalSrc) {
          /** @type {HTMLImageElement} */ (state.element).src = state.originalSrc;
        }
      }
    });
    ...
    videoStates.forEach((state) => {
      if (state && state.hadDataPoster) {
        state.element.setAttribute('data-poster', /** @type {HTMLVideoElement} */ (state.element).poster);
        if (state.originalPoster) {
          /** @type {HTMLVideoElement} */ (state.element).poster = state.originalPoster;
        }
      }
    });
  };
...
const getReadyToExport = (previewDom, cb) => {
  ...
  // 强制展开所有内容
  const restoreContent = expandAllContent(cherryPreviewer);
